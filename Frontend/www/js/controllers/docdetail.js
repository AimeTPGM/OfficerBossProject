...
.controller('DocumentDetailCtrl', function($scope, $stateParams,$ionicHistory, $http, $window, 
  ...
  $http.get('http://localhost:7777/documentDetail/'+$stateParams.folderId+'/'+$stateParams.docId)
  .then(function(resp){
    console.log(resp)
    $scope.doc = resp.data;
    $scope.creator = resp.data.creator;
    $scope.approverList = [];
    $scope.approverList = resp.data.approverList.approverIdList;
    $scope.file = resp.files;
